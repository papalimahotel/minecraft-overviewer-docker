...
RUN apt-get update && apt-get install -y git wget python3 python3-pip python3-pil nginx
...
# Create folders
RUN mkdir -p /ov/config && mkdir -p /ov/texture && mkdir -p /mc/world && mkdir -p ~/.minecraft/versions/$ver
...
RUN chmod +x /start && wget $url -O ~/.minecraft/versions/$ver/$ver.jar
